...
// Valyuta mezennesi
const apiUrl = 'https://api.exchangerate-api.com/v4/latest/AZN';
let exchangeRates = {};
...
async function fetchExchangeRates(){
    try{
        ...
        console.error('Mezenne gorsenmir', error)
    }
}
function convertCurrency(inputField, outField, inputSelect, outputSelect){
    const inputValue = parseFloat(inputField.value);
    const fromCurrency = inputSelect.value;
    const toCurrency = outputSelect.value;

    if(!isNaN(inputValue) && exchangeRates[toCurrency] && exchangeRates[fromCurrency]){
        const rate = exchangeRates[toCurrency] / exchangeRates[fromCurrency];
        outField.value = (inputValue * rate).toFixed(2);
    } else{
        outputField.value = '';
    }
}
document.addEventListener('DOMContentLoaded', async()=>{
   await fetchExchangeRates();
    const aznInput = getElementById("aznInput");
    const leftSelect = getElementById('left')
    const euroInput = getElementById("euroInput");
    const rightSelect = getElementById('right');

    aznInput.addEventListener('input', () => convertCurrency(aznInput, euroInput, leftSelect, rightSelect));
    euroInput.addEventListener('input', ()=> convertCurrency(euroInput, aznInput, rightSelect, leftSelect));
    leftSelect.addEventListener('change', ()=> convertCurrency(aznInput, euroInput, leftSelect, rightSelect));
    rightSelect.addEventListener('change', () => convertCurrency(euroInput, aznInput, rightSelect, leftSelect));
